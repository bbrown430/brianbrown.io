...
export const getHoverMutedColor = (color: ColorVariant) => colorMap[color].hoverMuted;
...
export const getColorClassesByString = (colorString: string): ColorConfig => {
  ...
  return {
    ...
    hoverMuted: `hover:bg-${colorString.replace('-400', '-600')}`, // Attempt to make it darker
    bg: `bg-${colorString}`,
...
export const generateColorVariants = (baseClasses: string, color: ColorVariant) => {
  ...
  return {
    ...
    hoverMuted: `${baseClasses} ${colors.hoverMuted} ${colors.border}`,
